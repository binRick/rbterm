/* Emoji rasterizer for macOS using Core Text + Core Graphics.
...
bool emoji_render(uint32_t codepoint, int pixel_size,
    ...
    CTFontRef font = CTFontCreateWithName(CFSTR("Apple Color Emoji"),
                                          (CGFloat)pixel_size, NULL);
    ...
    /* Big-endian byte order + premultiplied last = R,G,B,A in memory. */
    ...
    /* Flip so that (0,0) is top-left and drawing coordinates behave
     * intuitively from raylib's top-down perspective. */
    CGContextTranslateCTM(ctx, 0, size);
    CGContextScaleCTM(ctx, 1.0, -1.0);
    /* After the CTM flip, glyphs would be drawn upside-down — cancel it out
     * on the text matrix. */
    CGContextSetTextMatrix(ctx, CGAffineTransformMakeScale(1.0, -1.0));

    /* Centre the glyph by its bounding box. */
    CGRect bbox;
    ...
    /* With the CTM flip, y increases downward from the top. Baseline sits
     * at descent-from-top + bbox.height. We want glyph's top-left at
     * (size - bbox.height)/2. */
